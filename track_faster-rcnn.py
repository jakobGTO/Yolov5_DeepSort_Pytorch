...
def detect(opt):
    ...
    for frame_idx, (path, img, im0s, vid_cap, s) in enumerate(dataset):
        ...
        #########################################################
        #       INPUT FASTER RCNN DETECTIONS                    #
        #########################################################

        #fidx = frame_idx + 1
        #pred = np.loadtxt('../Faster-RCNN-CDReal-tresh09/seq2/det.txt', delimiter=',')
        pred = np.loadtxt("../Faster-RCNN-CDReal-tresh02/seq2/det.txt", delimiter=',')
        pred = pred[pred[:,0] == frame_idx, 1:]
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if det is not None and len(det):
                ...
                outputs = deepsort.update(xywhs, confs.cpu(), clss.cpu(), im0)
